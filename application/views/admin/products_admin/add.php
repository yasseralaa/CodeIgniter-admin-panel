...
<html>
  <head>
    ...
    <title>localhost/adminpanel TECH | ADD PRODUCT</title>
 ...
 <?php endif; ?>
					...
					<div class="col-lg-12">
						<div class="form-group">
							<label>Product Name</label>
                                                        <input class="form-control" type="text" name="product_name" value="<?php echo $this->session->flashdata('product_name');?>" placeholder="Enter Product Title" required = "required" />
						</div>
                                                
                                                <div  class="col-md-8 nopadding">
                                                    <label  class="block fg-label">Product Price</label>
                                                    <input  type="number" step="0.01" min="0" class="form-control" placeholder="Enter Product Price" name="product_price" value="<?php echo $this->session->flashdata('product_price');?>" required = "required" />
                                                    <p id ="product_price" class="fg-help red"></p>
                                                </div>
                                                <div class="col-md-4 ">
                                                    <label class="block fg-label">Currency</label>
                                                    <select id="product_currency" class="form-control" name="product_currency" required = "required">
		...
		<?php echo "<input type='file'  name='userfile' size='20' id='file' required = 'required'/>"; ?>
                                            ...
                                            foreach($categories as $struct) {
                                                if ($this->session->flashdata('category') == $struct->id) {
                                                    $cat = $struct;
                                                    break;
                                                }
                                            }
                                            
                                             foreach($subcategories as $struct) {
                                                if ($this->session->flashdata('category') == $struct->id) {
                                                    $cat = $struct;
                                                    break;
                                                }
						...
						<div class="form-group">
							...
							<select name="category" class="form-control" required = "required">
								 <option value="<?php if($cat==null)echo "";else echo $cat->id;?>" required><?php if($cat==null)echo "Select Category";else echo $cat->name;?></option>
								 <?php foreach($categories as $category) : ?>
								 	<option value="<?php echo $category->id; ?>" required = "required">*Category: <?php echo $category->name."   :-"; ?></option>
                  ...
                  <form>
                    <textarea id="editor1" name="product_description"  required = "required" rows="10" cols="80">
                                     <?php echo $this->session->flashdata('product_description');?>
                                                ...
                                                <input id="merchant_email" type="email" class="form-control" placeholder="Enter Merchant Mail" name="merchant_email" value="<?php echo $this->session->flashdata('merchant_email');?>" required = "required">
						...
						<div class="form-group">
                                                    <?php 
                                            $adm = null;
                                            foreach($admins as $struct) {
                                                if ($this->session->flashdata('admin') == $struct->id) {
                                                    $adm = $struct;
                                                    break;
                                                }
                                            }
							...
							<select name="admin"  class="form-control" required = "required">
                                                             <option value="<?php if($adm==null)echo "";else echo $adm->id;?>" required><?php if($adm==null)echo "Select Author";else echo $adm->first_name.' '.$adm->last_name;?></option>
